...
export const fetchGames = async ({ filters = {}, user = null } = {}) => {
  try {
    const query = qs.stringify(
      {
        filters:
          filters.onlyMyDevelopedGames && user
            ? {
                developed_by: {
                  id: {
                    $eq: user.id,
...
export const fetchUserPurchases = async () => {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `API Error! Status: ${response.status} - ${data?.error?.message || 'No message'}`
      );
    }
...
export const updateUserPassword = async (currentPassword, newPassword) => {
  try {
    ...
    if (!response.ok) {
      const errorMessage = data?.error?.message || data?.message || 'Ошибка при обновлении пароля';
